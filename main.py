...
# Imesmaeil1 | https://t.me/Imesmaeil1
# opened in ! { @CircuitSeed } !
...
app = Client('CircuitSeed', api_id, api_hash)
...
"""
        ...
        await message.reply("Please reply to a user's message to retrieve their info.")
...
async def enable_auto_reply(client, message):
    # فعال کردن پیام خودکار برای کاربر
    auto_reply_status[message.chat.id] = True
    await message.reply("پیام خودکار روشن شد.")

# دستور خاموش کردن پیام خودکار
@app.on_message(filters.command('sleepoff'))
async def disable_auto_reply(client, message):
    # غیرفعال کردن پیام خودکار برای کاربر
    auto_reply_status[message.chat.id] = False
    await message.reply("پیام خودکار خاموش شد.")

# ارسال پیام خودکار در صورت آفلاین بودن شما
@app.on_message(filters.private)
async def auto_reply(client, message):
    # چک کردن وضعیت پیام خودکار برای هر کاربر
    if auto_reply_status.get(message.chat.id, False):
        user_status = await client.get_chat(message.chat.id)
        
        # اگر شما آنلاین نباشید، پیام خودکار ارسال شود
        if user_status.status != "online":
            await message.reply("الان آنلاین نیستم، بعداً آنلاین شدم پیام میدم.")
